...
let browserInstance; // Persistent browser instance
...
async function scrapeFiverrProfile(profileUrl) {
  ...
  try {
    ...
    await new Promise(r => setTimeout(r, 5000)); // Wait for content to load
    ...
    const domData = await page.evaluate(() => {
      ...
      return {
        publicName: getText("h1[aria-label='Public Name']"),
        username: getText("div[aria-label='Username']"),
        gigTitle: getText("p[role='heading'][aria-level='3']"),
        reviewsCount: getText("#Reviews h2.text-display-7").split("Reviews")[0].trim() || "N/A",
      };
    });
    ...
    if (page) await page.close(); // Close page but keep browser open
  }
}

// API Route: Scrape Fiverr
app.post("/scrape", async (req, res) => {
  const { profileUrl } = req.body;
...
// Graceful Shutdown - Close Puppeteer on exit
